//! Wrapper of [`Tokenizer`](tokenizers_core::Tokenizer) with [`tokio`](tokio::fs) filesystem.

use tokenizers_core::TokenStreamBuilder;

pub use super::Error;
use crate::Charset;
use crate::io::tokio_io as io;
...
pub async fn tokenize_dir<T: TokenStreamBuilder>(
    ...
    'a: while let Some((read_dir, output_dir)) = stack.last_mut() {
        while let Some(entry) = read_dir.next_entry().await.map_err(Error::read_dir)? {
